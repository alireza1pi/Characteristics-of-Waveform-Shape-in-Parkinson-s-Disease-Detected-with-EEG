...
def loadPD(filepath='avg_data.mat',filepathrej='rejdata.mat'):
    '''
...
def measure_shape(eeg, rejects, boundaryS=100, ampPC=0, widthS=3, esrmethod='aggregate'):
    ...
    for s in range(S):
        ...
        newRejp=np.where(nn.isin(pks['off'][s],rejects['off'][s]))
        peaksharp=np.delete(pksharp['off'][s],newRejp)
        risesteep=np.delete(risteep['off'][s],newRejp)
        newRejt=np.where(nn.isin(trs['off'][s],rejects['off'][s]))
    ...
    for s in range(S):
        ...
        #remove artifacts
        newRejp=np.where(nn.isin(pks['on'][s],rejects['on'][s]))
        peaksharp=np.delete(pksharp['on'][s],newRejp)
        risesteep=np.delete(risteep['on'][s],newRejp)
        newRejt=np.where(nn.isin(trs['on'][s],rejects['on'][s]))
    ...
    for s in range(Sc):
        ...
        #remove artifacts
        newRejp=np.where(nn.isin(pks['C'][s],rejects['C'][s]))
        peaksharp=np.delete(pksharp['C'][s],newRejp)
        risesteep=np.delete(risteep['C'][s],newRejp)
        newRejt=np.where(nn.isin(trs['C'][s],rejects['C'][s]))
